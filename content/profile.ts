...
export const summary: Record<CvVariant, I18nText> = {
  recruiter: {
    es: 'Desarrollador front-end en Cantabria, con casi una década construyendo interfaces que aguantan producción. Empecé en ERPs industriales —distribución eléctrica, automoción, petroquímica— y desde 2021 me dedico a experiencias 3D en el navegador: showrooms y configuradores con Babylon.js sobre Vue 3 y Nuxt. Trabajo con agentes de IA integrados en el flujo diario, no como demo: revisión, refactor y documentación con criterio propio sobre lo que devuelven. Fuera del horario mantengo sistemas completos de punta a punta: un e-commerce con Nuxt 4 y Medusa, dos sistemas de trading algorítmico en Python con su propia suite de tests y un editor RAW local cuyo motor operan por igual la interfaz Vue y un agente de IA vía MCP.',
    en: 'Front-end developer based in northern Spain, with close to a decade of shipping interfaces that hold up in production. I started on industrial ERPs — power distribution, automotive, petrochemical — and since 2021 I have focused on 3D experiences in the browser: showrooms and configurators built with Babylon.js on Vue 3 and Nuxt. AI agents are part of my daily workflow rather than a demo: review, refactor and documentation, with my own judgement applied to whatever comes back. Off the clock I run whole systems end to end: a Nuxt 4 + Medusa storefront, two algorithmic trading systems in Python with their own test suite, and a local RAW editor whose engine answers equally to its Vue interface and to an AI agent over MCP.',
  },
  anon: {
